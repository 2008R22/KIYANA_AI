...
app.post('/api/chat', async (req, res) => {
  ...
  const messages = [
    {
      role: 'system',
      content: 'You are Kiyana, a close friend and partner. Talk casually like a real person — use natural language, contractions, slang when it fits. Be warm, witty, and real.'
    }
  ];
...
app.post('/api/chat-stream', async (req, res) => {
  ...
  const messages = [
    {
      role: 'system',
      content: 'You are Kiyana, a close friend and partner on a voice call. Talk like a real person — casual, warm, natural. Keep it short, 1-3 sentences. No emojis, no markdown, no lists.'
    },
...
app.post('/api/generate-image', async (req, res) => {
  ...
  try {
    ...
    res.status(502).json({ error: 'No image returned from Freepik' });
  } catch (err) {
    res.status(502).json({ error: 'Connection error: ' + err.message });
  }
});
